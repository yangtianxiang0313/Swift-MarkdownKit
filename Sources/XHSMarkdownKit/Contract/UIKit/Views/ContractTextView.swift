...
public final class ContractTextView: UIView, HeightEstimatable, StreamableContent {
    ...
    public func estimatedHeight(atDisplayedLength: Int, maxWidth: CGFloat) -> CGFloat {
        ...
        if atDisplayedLength >= attrString.length {
            ...
            displayedString = attrString.attributedSubstring(
                from: NSRange(location: 0, length: max(0, atDisplayedLength))
            )
        }
    ...
    public func reveal(upTo length: Int) {
        ...
        if clampedLength >= attrString.length {
            ...
            textView.attributedText = attrString.attributedSubstring(
                from: NSRange(location: 0, length: clampedLength)
            )
        }
    }
    ...
    public override func layoutSubviews() {
        ...
        textView.frame = CGRect(
            x: indent,
            y: 0,
            width: max(0, bounds.width - indent),
