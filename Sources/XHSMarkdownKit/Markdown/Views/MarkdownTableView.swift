...
public final class MarkdownTableView: UIView, HeightEstimatable {
...
// MARK: - TableContentView

private final class TableContentView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        ctx.fill(CGRect(x: 0, y: 0, width: bounds.width, height: rh))

        drawRow(ctx: ctx, cells: data.headers, alignments: data.alignments, y: 0, rowHeight: rh)

        for (row, rowData) in data.rows.enumerated() {
            let y = CGFloat(row + 1) * rh
            if row % 2 == 1 {
                ctx.setFillColor(tableStyle.headerBackgroundColor.withAlphaComponent(0.3).cgColor)
                ctx.fill(CGRect(x: 0, y: y, width: bounds.width, height: rh))
            }
            drawRow(ctx: ctx, cells: rowData, alignments: data.alignments, y: y, rowHeight: rh)
        }
        ...
        for w in columnWidths {
            ...
            ctx.addLine(to: CGPoint(x: x, y: CGFloat(totalRows) * rh))
            x += w
        }
        ctx.move(to: CGPoint(x: bounds.width, y: 0))
        ctx.addLine(to: CGPoint(x: bounds.width, y: CGFloat(totalRows) * rh))

        ctx.strokePath()
    }

    private func drawRow(ctx: CGContext, cells: [NSAttributedString], alignments: [TableColumnAlignment], y: CGFloat, rowHeight: CGFloat) {
        var x: CGFloat = 0
        for (col, cell) in cells.enumerated() where col < columnWidths.count {
            ...
            let drawX: CGFloat
            switch alignment {
            case .center: drawX = x + padding.left + (availableW - ceil(size.width)) / 2
            case .right:  drawX = x + colW - padding.right - ceil(size.width)
            default:      drawX = x + padding.left
            }
