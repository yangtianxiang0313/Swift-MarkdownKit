...
public final class BlockQuoteContainerView: UIView, FragmentContaining, HeightEstimatable, StreamableContent {
    ...
    public func estimatedHeight(atDisplayedLength: Int, maxWidth: CGFloat) -> CGFloat {
        ...
        for (i, fragment) in fragments.enumerated() {
            ...
            if let estimatable = view as? HeightEstimatable {
                totalHeight += estimatable.estimatedHeight(atDisplayedLength: fragmentDisplayed, maxWidth: contentWidth)
            } else {
    ...
    public override func layoutSubviews() {
        ...
        contentView.frame = CGRect(x: offset, y: 0, width: max(0, bounds.width - offset), height: bounds.height)
        ...
        layoutCoordinator.relayout(
            ...
            displayedLengthProvider: { fragment in
                return revealMap[fragment.fragmentId] ?? self.fragmentLength(fragment)
            }
        )
    }
